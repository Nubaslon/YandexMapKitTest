...
@property (nonatomic, strong) PointAnnotation * annotationKremlin;
...
- (void)configureAndInstallAnnotations {
    ...
    self.yacAnnotation.title = @"МоскваСити";
    
    self.annotationKremlin = [PointAnnotation pointAnnotation];
    self.annotationKremlin.coordinate = YMKMapCoordinateMake(55.752453, 37.617383);
    self.annotationKremlin.title = @"Кремль";

    self.annotationKremlin.subtitle = @"Центр столицы";

    [self.mapView addAnnotation:self.annotationKremlin];
...
-(YMKCalloutView *)mapView:(YMKMapView *)view calloutViewForAnnotation:(id<YMKAnnotation>)annotation
{
    ...
    YMKDefaultCalloutView * callout = (YMKDefaultCalloutView *)[view dequeueReusableCalloutViewWithIdentifier:identifier];
    if (callout == nil) {
...
- (void)configureAnnotationView:(YMKPinAnnotationView *)view forAnnotation:(id<YMKAnnotation>)annotation {
    ...
    else if (annotation == self.annotationKremlin) {
        view.image = [UIImage imageNamed:@"kremlin.jpg"];
